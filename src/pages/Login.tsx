...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = mockAuth.login(formData.email, formData.password);
      if (result.success) {
        toast({
  ...
  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      const result = mockAuth.loginWithGoogle();
      if (result.success) {
        toast({
    ...
    <div className="min-h-[calc(100vh-180px)] flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Faça login para acessar sua conta na Praça da Ciência
          </CardDescription>
        </CardHeader>
